...
from accnts.views import is_user, user_login_required
...
def show_select(request):
    if request.method == "GET" and len(request.GET) != 0:
        ...
        films = ""
        # add showtitme >= current time + 5 min
        shows = show.objects.filter(end_date__gte=date, start_date__lte=date).select_related('movie_id', 'movie__url',
                                                                                             'movie__movie_name').order_by(
            'movie_id', 'showtime').values_list('id', 'price', 'showtime', 'movie', 'movie__url', 'movie__movie_name',
                                                named=True)
        res_dict = {}

        # Grouping shows rows by movie and appending showtitmes in a list
        for s in shows:
...
def send_mail_page(request, message):
    context = {}
...
def checkout(request):
    ...
    if request.method == "POST":
        ...
        booking.objects.create(booking_code="Random", user=request.user, show=showinfo, show_date=show_date,
        ...
        send_mail_page(request, message)
...
def userbookings(request):
    ...
    booking_table = booking.objects.filter(user=request.user).select_related().order_by('-booked_date').values_list(
        'id', 'show_date', 'booked_date', 'show__movie__movie_name', 'show__movie__url', 'show__showtime', 'total',
        'seat_num', named=True)
...
def cancelbooking(request, id):
    ...
    message = "\nYour tickets are successfully Cancelled. Here are the details.\nYour show info{}\nYour Show date {}\nYour seats\n\nThank you,\nBookMyTicket".format(
