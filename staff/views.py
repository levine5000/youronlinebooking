from accnts.models import Account
from accnts.views import is_user, staff_required
...
def index(request):
    ...
    b_data = booking.objects.filter(show_date__gte=start_date).values_list('show_date').order_by('show_date').annotate(
        total_seats=Sum('num_seats'))
    r_data = booking.objects.filter(show_date__gte=start_date).values_list('show_date').order_by('show_date').annotate(
        total=Sum('total'))

    booking_table = booking.objects.select_related().order_by('-booked_date').values_list('show_date', 'booked_date',
                                                                                          'show', 'total', 'num_seats',
                                                                                          'show__movie__movie_name',
                                                                                          'show__showtime',
                                                                                          'user__username', named=True)
...
@user_passes_test(staff_required, login_url='/accounts/adminlogin')
def movies(request):
    movies = film.objects.filter().order_by('-id').values_list('id', 'movie_name', 'movie_genre', 'date_added',
                                                               'movie_lang', 'url', 'movie_year', named=True)
...
@user_passes_test(staff_required, login_url='/accounts/adminlogin')
def banners(request):
    banner_data = banner.objects.all().select_related().values_list('id', 'movie__movie_name', 'url', 'modified',
                                                                    named=True)
